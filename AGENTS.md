...
- Keep the default installation telemetry-only. Any write/control feature must be separately reviewed, explicitly opt-in, serialize conflicting actions, revalidate current options and fresh authoritative state in the action path, issue at most one request with transport redirects disabled and no replay after an ambiguous result, require post-command reported-state confirmation, and ship with transaction-level tests.
